...
from utils import html2md, HashableDict, print
import utils
import sqlite3
...
def clean_item(item):
    ...
    if "attachments" in item:
        for att in item["attachments"]:
            if "path" in att:
                att["path"] = re.sub(
                    r".*/zotero/storage/[^/]+", "ATTACHMENT_KEY", att["path"]
                )
            att.pop("uri", None)
...
def unkey(data):
    if isinstance(data, list):
        ...
        return {
            ...
            if k not in ("itemID", "itemKey", "key")
        }
...
def load(lib):
    ...
    lib["items"] = sorted(
        lib["items"], key=lambda i: json.dumps(unkey(i), sort_keys=True)
    )
    ...
    for coll in lib["collections"].values():
        for sub in coll.get("collections", []):
            db.execute(
                "UPDATE collections SET parent = ? WHERE id = ?", (coll["key"], sub)
            )
    ...
    collectionKeys = {}
    for coll in sorted(
        lib["collections"].values(), key=lambda c: collectionName(c["key"])
    ):
    ...
    lib["collections"] = {
        coll["key"]: coll
        for coll in sorted(
            lib["collections"].values(),
            key=lambda c: json.dumps(unkey(c), sort_keys=True),
        )
    }
